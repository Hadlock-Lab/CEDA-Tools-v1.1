...
# DBTITLE 1,Get diagnoses by encounter
...
def get_diagnoses_by_encounter(encounter_table_name, cc_dictionary_file, cc_label_list):
  ...
  encounter_df = spark.sql("SELECT * FROM rdp_phi_sandbox.{}".format(encounter_table_name))
...
# DBTITLE 1,Get diagnoses by patient
...
# DBTITLE 1,Get medications by encounter
...
# DBTITLE 1,Get medications by patient
...
# DBTITLE 1,Get labs by encounter
...
# DBTITLE 1,Merge patient ML tables
...
# DBTITLE 1,Merge encounter ML tables
...
# DBTITLE 1,Generate ML features table by encounter
def generate_ml_features_table_with_race_by_encounter(condition_feature_defs, med_feature_defs, lab_feature_defs, ml_encounter_table):
  ...
  ## Qi added the following codes for age feature
  age_feature_col1 = F.when( (F.col('age') >= 18.0) & (F.col('age') < 50.0), 1).otherwise(0).alias('age_group_18_49')
  age_feature_col2 = F.when( (F.col('age') >= 50.0) & (F.col('age') < 75.0), 1).otherwise(0).alias('age_group_50_74')
  age_feature_col3 = F.when( (F.col('age') >= 75.0), 1).otherwise(0).alias('age_group_above_75')
  ...
  ## Orginal with age feature col
  feature_table_df = ml_encounters_df \
    .select(*non_feature_col_names, age_feature_col1, age_feature_col2, age_feature_col3, sex_feature_col, ethnic_group_feature_col, *derived_feature_cols)
  
  return feature_table_df

# COMMAND ----------

# DBTITLE 1,Generate ML features table by patient
